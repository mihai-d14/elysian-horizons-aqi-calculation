def calculate_air_quality_score(aqi: float) -> int:
    """
    We convert the EPA AQI (0-500+) to 0-100 score where:
    - EPA AQI 0-3 → score 100 (best)
    - Each +3 EPA AQI points → -1 score point
    - EPA AQI ≥ 300 → score 0 (hazardous)
    
    Score ranges are mapped to EPA categories:
    ...
    # For other levels, each 3 point increase in AQI = 1 point decrease in score
    score = 100 - (aqi / 3)
    
    # Ensuring score is between 0 and 100
    score = max(0, min(100, score))
    
    return round(score)  # Returning a rounded integer
